...
Token* tokenIdentifier(Lexer* this, Token* token){
    ...
    *ptr = '\0';
    ...
    if(strcmp(token->word, "main") == 0){
        token->type = TOKEN_MAIN;
        return token;
    }

    if(strcmp(token->word, "function") == 0){
    ...
    if(strcmp(token->word, "infinitWhile") == 0){
        token->type = TOKEN_INFWHILE;
        return token;
    }
    ...
    if(strcmp(token->word, "continue") == 0){
        token->type = TOKEN_CONTINUE;
        return token;
    }
    if(strcmp(token->word, "true") == 0 || strcmp(token->word, "false") == 0){
